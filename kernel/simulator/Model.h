 ...
 * A model is mainly represented by a collection of components (ModelComponent), adequately configurated and connected, and a collection of under layered modeldatum (ModelData).
 */
class Model {
public:
...
public: // model control
	...
	void clear();
	void show();
	bool insert(ModelData* elemOrComp); ///< Insert a new ModelData or ModelComponent into the model (since 20191015). It's a generic access to ComponentManager->insert() or ModelDatao->insert()
	void remove(ModelData* elemOrComp); ///< Remove a new ModelData or ModelComponent into the model (since 20191015). It's a generic access to ComponentManager->remove() or ModelDatao->remove()
...
public: // only gets	
	...
	ModelDataManager* getData() const; ///< Provides access to the class that manages the most basic elements of the simulation model (such as queues, resources, variables, etc.).
