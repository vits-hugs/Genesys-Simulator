...
StatisticsCollector::StatisticsCollector(Model* model, std::string name, ModelDataDefinition* parent, bool insertIntoModel) : ModelDataDefinition(model, Util::TypeOf<StatisticsCollector>(), name, insertIntoModel) {
    ...
    _parentModel->getResponses()->insert((PropertyGetterBase*)new PropertyGetterDouble(Util::TypeOf<StatisticsClass>(), getName() + ".average",
            DefineGetterDouble<StatisticsClass>(static_cast<StatisticsClass*>(this->_statistics),  &StatisticsClass::average), getName()));
    _parentModel->getResponses()->insert((PropertyGetterBase*)new PropertyGetterDouble(Util::TypeOf<StatisticsClass>(), getName() + ".halfwidth",
            DefineGetterDouble<StatisticsClass>(static_cast<StatisticsClass*>(this->_statistics),  &StatisticsClass::halfWidthConfidenceInterval), getName()));
}
