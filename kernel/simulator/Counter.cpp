...
Counter::Counter(Model* model, std::string name, ModelDataDefinition* parent) : ModelDataDefinition(model, Util::TypeOf<Counter>(), name) {
    ...
    // NEW !!
    model->getResponses()->insert((PropertyGetterBase*)new PropertyGetterDouble(Util::TypeOf<Counter>(), getName(),
            DefineGetterDouble<Counter>(this, &Counter::getCountValue)) );
}
