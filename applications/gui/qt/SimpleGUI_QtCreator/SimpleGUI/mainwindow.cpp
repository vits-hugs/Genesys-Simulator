...
std::string MainWindow::_recursiveCreateModelGraphicPicture(ModelComponent* component, std::list<ModelComponent*>* visited) {
    ...
    if (ui->checkBox_ShowInternals->isChecked()) {
        ...
        std::string dataname;
        int pos;
        for (std::string internalKey: *component->getInternalDatasKeys()) {
            data = component->getInternalData(internalKey);
            dataname = data->getName();
            pos = dataname.find(".", 0);
            while (pos < dataname.length()) {// != std::string::npos) {
                dataname= dataname.replace(pos, 1, "_");
                pos = dataname.find(".", 0);
            }
            result += "  "+dataname+" "+ "[shape=record, fontsize=9, color=gray50, fontcolor=gray50, label=\"{"+data->getClassname()+"|"+dataname+"}\"]" + ";\n";
            result += "    "+dataname +"->"+ component->getName()+" [style=dashed, arrowhead=\"ediamond\", color=gray50];\n";
        }
    }
...
bool MainWindow::_createModelGraphicPicture() {
    ...
    std::list<ModelComponent*>* visited = new std::list<ModelComponent*>();
    ...
    try {
        ...
        try {
            ...
            ui->label_ModelGraphic->setScaledContents(true);
