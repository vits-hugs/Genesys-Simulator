...
std::string MainWindow::_recursiveCreateModelGraphicPicture(ModelComponent* component, std::list<ModelDataDefinition*>* visited) {
    std::string separator = "_";
    std::string result = "  " + component->getName() + " " + "[shape=record, fontsize=12, style=filled, fontcolor=black, fillcolor=bisque, label=\"{" + component->getClassname() + "|" + component->getName() + "}\"]" + ";\n";
    visited->insert(visited->end(), component);
    std::list<ModelDataDefinition*>::iterator visitedIt;
    ModelDataDefinition* data;
    std::string dataname;
    if (ui->checkBox_ShowInternals->isChecked()) {
        for (std::pair<std::string, ModelDataDefinition*> data : *component->getInternalData()) {
            dataname = Util::StrReplaceAll(data.second->getName(), ".", "_");
            visitedIt = std::find(visited->begin(), visited->end(), data.second);
            if (visitedIt == visited->end()) {
                result += "  " + dataname + " " + "[shape=record, fontsize=8, color=gray50, fontcolor=gray50, label=\"{" + data.second->getClassname() + "|" + data.second->getName() + "}\"]" + ";\n";
            }
            result += "    " + dataname + "->" + component->getName() + " [style=dashed, arrowhead=\"diamond\", color=gray50, fontcolor=gray50, fontsize=7, label=\"" + data.first + "\"];\n";
        }
    }
    if (ui->checkBox_ShowElements->isChecked()) {
        for (std::pair<std::string, ModelDataDefinition*> data : *component->getAttachedData()) {
            dataname = Util::StrReplaceAll(data.second->getName(), ".", "_");
            visitedIt = std::find(visited->begin(), visited->end(), data.second);
            if (visitedIt == visited->end()) {
                result += "  " + dataname + " " + "[shape=record, fontsize=9, color=gray50, fontcolor=gray50, label=\"{" + data.second->getClassname() + "|" + data.second->getName() + "}\"]" + ";\n";
            }
            result += "    " + dataname + "->" + component->getName() + " [style=dashed, arrowhead=\"ediamond\", color=gray50, fontcolor=gray50, fontsize=7, label=\"" + data.first + "\"];\n";
        }
    }
...
bool MainWindow::_createModelGraphicPicture() {
    ...
    try {
        ...
        savefile.open("./_temp.dot", std::ofstream::out);
        ...
        try {
            system("dot -Tpng ./_temp.dot -o ./_temp.png");
...
void MainWindow::on_actionLicence_triggered() {
    ...
    text+= "Limits are entities="+std::to_string(licman->getEntityLimit())+", components="+std::to_string(licman->getModelComponentsLimit());
    text+= ", datadefinitions="+std::to_string(licman->getModelDatasLimit());
