	...
	if (plugin != nullptr) {
		if (plugin->isIsValidPlugin()) {
			...
			if (plugin->getPluginInfo()->isComponent()) {
				...
				treeItemChild->setIcon(0, QIcon(":/icons3/resources/icons/pack3/ico/calendar.ico"));
			...
			if (founds.size() == 0) {
				...
				QBrush bforeground(Qt::yellow);
				treeItem->setForeground(0, bforeground);
				QBrush bbackground(Qt::black);
				treeItem->setBackground(0, bbackground);
