...
Queue::Queue(Model* model, std::string name) : ModelDataDefinition(model, Util::TypeOf<Queue>(), name) {
    PropertySetterBase* prop1= (PropertySetterBase*)new PropertySetterString(Util::TypeOf<Queue>(), "Attribute name",
                                                  DefineGetterString<Queue>(this, &Queue::getAttributeName),
                                                  DefineSetterString<Queue>(this, &Queue::setAttributeName));
    _addProperty(prop1);
    //@TODO: OrderRule!!
}
