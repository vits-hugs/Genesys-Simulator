...
int Smart_Failures::main(int argc, char** argv) {
    ...
    Failure* failure1 =  plugins->newInstance<Failure>(model);
    failure1->setUpTimeExpression("50");
    failure1->setDownTimeExpression("25");
    Failure* failure2 =  plugins->newInstance<Failure>(model);
    failure2->setFailureType(Failure::FailureType::COUNT);
    failure2->setCountExpression("10");
    failure2->setDownTimeExpression("20");
	Resource* machine1 = plugins->newInstance<Resource>(model, "Machine_1");
    machine1->getFailures()->insert(failure1);
    machine1->getFailures()->insert(failure2);
